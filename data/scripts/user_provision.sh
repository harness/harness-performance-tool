...
password=<password>
localLogin=false
# For localLogin=true enable FF : AUTO_ACCEPT_SAML_ACCOUNT_INVITES to add users without invitation
# Note: Users added with SAML enabled cannot set user password
...
loginPath=login
if [ "$localLogin" == "true" ]; then
  loginPath=harness-local-login
fi

# login 
b64Str=$(printf "%s" "$username:$password" | base64)
login_result_response=$(curl -s --location "$url/gateway/api/users/$loginPath" \
...
for ((index = 0; index < $userCount; index++)); do
    ...
    if [ "$localLogin" != "true" ]; then
        ...
        echo "Signup response for $email: $signup_user_response"
    fi
